 ...
 * (slow-not-failed). 제어 값은 {@code response.delay.refresh-interval-ms}(기본 2초)
 * 마다 한 번만 읽어 캐시하고, 조회 실패·행 부재는 지연 0 으로 폴백한다 — 제어 표면이
 * 죽어도 앱은 정상이어야 한다. 상한(10000ms)은 DDL 의 CHECK 와 같은 값이다.
 */
@Component
