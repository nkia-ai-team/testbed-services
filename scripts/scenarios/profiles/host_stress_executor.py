...
# 2026-08-11(6차 run f836678b): 사다리가 메모리 축에서 **역전**돼 있었다. 위 ①의
# `--vm-keep` 수리가 적용되고 러너 게이지도 `system_mem_utilization`으로 고쳐진 뒤의
# 첫 깨끗한 실측이다 — 그래서 위 "재실행으로 다시 세워야 한다"가 이제 세워졌다.
#
#   0단 vm1x1500M   30.6 -> 42.8%   (+12.3pp, 명목 1500MiB ≈ 12.6pp)
#   1단 vm1x3000M         -> 58.9%  (+25.0pp, 명목 3000MiB ≈ 25.2pp)
#   2단 vm2x2500M         -> 51~54% (+21~24pp)   ← 중간단보다 **낮다**
#
# vm1 단 둘은 명목치를 소수점까지 선형으로 전달하는데 vm2 단만 워커 하나치(~2,500MiB)
# 어치만 상주한다. cpu_workers 4 가 4코어를 전부 점유해 두 vm 워커가 `--vm-keep`의
# redirty 루프를 돌릴 CPU 를 얻지 못하고 페이지를 회수당한 것으로 본다(가설). 확정된
# 것은 기전이 아니라 "올라갈수록 메모리가 줄어든다"는 실측 자체다. 6차 노트가 최상단을
# ~72% 로 예측한 산술은 2x2500=5000 을 가정했고 이 실측이 그것을 반증한다.
#
# 총 바이트(5000MiB, eviction 무릎 아래)도 정체성(cpu4 + 메모리 압박)도 그대로 두고
# 워커 수만 1 로 되돌린다. 예측(가설): 약 71.5%. 7차 배치가 잰다.
#
# ⚠️ 이 수리만으로 F15-P 가 통과하지는 않는다. success 는 네 조건의 AND 이고
# 나머지 둘(`transfer_p95 >= 4000` / `account_p95 >= 1000`)이 6차 실측 최대
