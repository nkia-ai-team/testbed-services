...
CONTRACTS: dict[str, dict[str, Any]] = {
    ...
    # 못한다(07-20 실측, payments 357k행 상주). tagged 세션이 payments 테이블을
    # EXCLUSIVE MODE로 잡아 INSERT(ROW EXCLUSIVE)를 블록하되 평문 SELECT/actuator
    # health(SELECT 1·ACCESS SHARE)는 통과시켜 pod readiness 자기-abort를 피한다.
    "F06-H": {"engine": "postgresql", "namespace": "rca-testbed-commerce", "pod": "testbed-postgres-0", "schema": "payment_schema", "table": "payments", "lock_scope": "table", "lock_mode": "EXCLUSIVE", "application_name": "rca-F06-H-payment-lock", "hold_seconds": 600},
}
...
def validate(scenario_id: str, params: dict[str, Any], profile: dict[str, Any]) -> None:
    ...
    # 세션 태그는 rca-<scenario>-<surface>-lock 형태로 시나리오를 결속한다. surface
    # 접미사(inventory/payment/...)는 하드코딩하지 않고 파라미터화한다 — F01-R의
    # inventory-lock 계약을 그대로 통과시키면서 payment-lock 등으로 일반화한다.
    app_name = params.get("application_name", "")
    if not app_name.startswith(f"rca-{scenario_id}-") or not app_name.endswith("-lock"):
...
def build_invocation(plan: dict[str, Any], action: str) -> tuple[list[str], bytes]:
    ...
    if p.get("engine") == "postgresql":
        return ["/usr/bin/bash", "-s", "--", action, plan["scenario"]["id"], p["namespace"], p["pod"], p["schema"], p["table"], p["lock_mode"], p["application_name"], str(p["hold_seconds"])], POSTGRES_POD_REMOTE
...
set -euo pipefail
action="$1"; scenario="$2"; ns="$3"; pod="$4"; schema="$5"; table="$6"; mode="$7"; tag="$8"; hold="$9"
# Only allowlisted lock modes reach SQL. EXCLUSIVE blocks INSERT (ROW EXCLUSIVE)
# while leaving plain SELECT (ACCESS SHARE) free (e.g. actuator health SELECT 1),
# so payment pod readiness does not self-abort the run.
case "$mode" in EXCLUSIVE|"ACCESS EXCLUSIVE"|"SHARE ROW EXCLUSIVE") : ;; *) echo "unsupported lock mode: $mode" >&2; exit 2 ;; esac
...
case "$action" in
 ...
 run) table_ok; [[ "$(count)" == 0 ]]; "${k[@]}" exec "$pod" -- "${envs[@]}" sh -lc 'nohup env PGAPPNAME="$TAG" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -c "BEGIN; LOCK TABLE $SCHEMA.$TABLE IN $MODE MODE; SELECT pg_sleep($HOLD); ROLLBACK;" >"/tmp/$TAG.log" 2>&1 </dev/null &' ;;
 cleanup) stop; "${k[@]}" exec "$pod" -- rm -f "/tmp/$tag.log" ;;
 recovery) [[ "$(count)" == 0 ]]; table_ok ;;
 *) exit 2 ;;
esac
'''
