...
lucida-next / OpenTelemetry 기반으로 관측되는 Spring Boot 마이크로서비스 모음이며, 각 도메인은 K3s 위에 배포되어 RCA 시나리오 검증 대상이 된다.
인프라(배포 자동화)가 본 레포 URL 을 `app_repo` 로 받아 K3s 에 배포하고, 각 도메인 폴더의 `k8s/build-and-deploy.sh` 를 진입점으로 호출한다.
