...
- 고정 경로 2개 (둘 다 commerce-payment → core-banking-transfer, `POST /api/transfers`, FQDN `testbed-transfer.rca-testbed-banking.svc.cluster.local:8082`):
  ① 결제 시점 동기 이체(checkout 마다), ② 매시 정산 배치 이체(미정산 집계분).
  계좌 계약: `commerce-settlement`(from)·`commerce-merchant`(to)는 banking 시드에 실존해야 한다.
