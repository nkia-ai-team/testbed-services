...
tags:
  ...
  - rca
  - evaluation
summary: RCA agent 평가를 위한 양성·음성 시나리오의 카탈로그 구조, 설계 템플릿, 실행 시간 구조, golden 승격 기준, 시나리오당 파일 레이아웃을 정의한다.
---
...
**선정 원칙 — RCA에 국한하지 않는다(2026-07-15).** 이 백로그는 RCA 데이터
소스 갭에서 도출됐지만, Phase 1 후보 선정은 **AIOps 파이프라인 전체**
(이상감지 → 이벤트/클러스터 → 인시던트 격상 → RCA → 챗봇,
[평가용 데이터 캡처 §8](spec-eval-data-capture.md) 모듈 표)에 대한 가치로
평가한다. 좋은 시나리오는 이 사슬을 끝까지 통과시킨다 — 이상 이벤트가
여럿 발생하고, 한 사건으로 묶이고, 인시던트로 격상되고, 원인이 추적되고,
챗봇 질문의 소재가 된다. 추가 기준: **실전 발생 빈도**(운영 환경에서 흔한
장애 우선). 후보별 "어느 모듈까지 발현시키는가"는 선정 시 명시한다.
...
`injection`에 기록한다 — 유형과 위치가 어긋나면 topology가 비현실적이 된다.
...
| F3 | **15분 타임라인 vs 인시던트 judge 시계 불일치** — judge는 3m close/10m 보류/최대 30m 유예로 판정이 §3 타임라인보다 늦는 게 정상. 첫 캡처 "인시던트 미승격"도 이와 연관 가능 | eval-capture §8.2 대조 | 주입 종료와 평가 종료 분리 — `pipeline_settle_window`(30~45m) 신설, §3와 R6 재조정 |
...
- `pipeline_settle_window` 길이와 §3 타임라인·R6 간격 재조정(§9.3 F3).
