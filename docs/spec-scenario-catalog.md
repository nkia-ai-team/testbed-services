...
4. F02·F03·F07·F08을 추가하고 F12·F13 선행조건을 점검해 Phase 1 후보를
   전체 60개에서 균형 있게 선정한다. → **완료(2026-07-20, §8)**
5. 테스트베드 총용량 결과를 받아 각 후보의 강도·지속시간·중단 임계치를 붙인다.
6. Phase 2는 NMS/WPM·기능 정합성의 선행 계약을 먼저 만든 뒤 상세화한다.

## 8. Phase 1 최종 선정 (2026-07-20 확정)

캡처 계약 v2([평가용 데이터 캡처 §2.1](spec-eval-data-capture.md)) 전면
재시작 방침에 따라, 신계약 하에서 재실행할 Phase 1 목록을 30개(+조건부 1)로
확정한다.

### 8.1 1군 — 라이브 승격 20개 (자동 포함, 구현·검증 이력 보유)

F01-R · F01-H · F01-G · F02-R · F03-G · F04-R · F05-R · F05-H · F05-G ·
F06-R · F06-G · F07-H · F07-P · F08-H · F08-P · F09-P · F09-R · F11-R ·
F11-G · F12-H

### 8.2 2군 — 추가 10개 (커버리지 갭을 메우는 partial 후보, 상세화 필요)

| ID | 메우는 갭 | 남은 준비 작업 |
| --- | --- | --- |
| F01-P | cross-domain 전파 + Oracle DPM (최우선) | Oracle lock 스크립트 |
| F08-G | distractor·anti-merge | F01-P Oracle 스크립트 재사용 |
| F15-G | multi-root(단일 원인 확정 부당) | 스크립트 재사용 + row·순서 확정 |
| F15-T1 | 동시 이종 장애의 시간 경계 | food OOM 안전 강도 실측 |
| F15-R | flapping·reopen episode 경계 | judge episode 간격 실측 |
| F06-H | F06-R의 H-감별(외부 timeout vs DB 잠금) | payments 테이블 잠금 표면 상세화(07-20 실측 완료) |
| F03-P | 설정 변경형 pool 고갈(통제형) | pool 경계·강도 실측 |
| F03-H | thread 고갈 감별(DB 장애처럼 보임) | 강도 실측(executor 메트릭 간접 주의) |
| F05-P | 공통 인프라 root 다서비스 merge | eviction 안전 강도 실측(배치 지도 확보됨) |
| F09-H | SMS/JVM 축(GC 압박) | OOM 전 pause 범위 실측 |

### 8.3 조건부 +1과 명시적 미완성

- **F02-P(MySQL 인덱스, 조건부 31호)**: food menus 시드 보강(수만 행)을
  선행하면 포함한다 — MySQL 축(§5.3 DB 3종 요건)의 유일한 현실적 후보.
- **커버리지 미완성 표시(§2 규정)**: NMS(F12-R·G — .57 셋업 승인 대기),
  WPM(F13 전체 — probe 표면 부재), 디스크(F10 — device 미확정)는 선행조건
  미해소로 Phase 1에서 제외하며 해당 계열 커버리지는 미완성이다.
