...
> **후속 (2026-08-07, [41종 진단·수리 기록](batch-41-live-findings-0807.md))**
>
> 아래 항목 중 일부는 그 뒤 실측으로 **결론이 바뀌었다.** 이 문서만 읽고 낡은
> 판정을 그대로 믿지 말 것.
>
> | 이 문서 | 당시 판정 | 08-07 실측 결과 |
> |---|---|---|
> | #5 F05-R | "G3 피해 부재" | **오진.** 피해가 없던 게 아니라 commerce 재고 고갈로 checkout이 409에 걸려 payment에 도달하지 못했다(0807 #2a·#10) |
> | #23 F06-P | "성공↔감별자 상호배타" | **오진.** 감별자가 주입과 무관한 dispatch 노이즈(43/439≈0.098)를 세고 있었다(0807 #9) |
> | #25 F10-H | "같은 레버가 노드마다 다르다" | **보강.** 노드 차이보다 판정 페이스가 결정적이었다 — 60s 케이던스 p95가 gate를 묶었다(0807 #5) |
> | #17 F20-R | "엣지에서 빨리 실패해 하류가 안 느려짐" | **보강.** 주입 자신이 must_rule_out을 위반하고 있었다(rps 30에서 전면 500). 사다리 강등(0807 #14) |
> | #22 F03-H | "60rps로도 포화가 안 된다" | **해소.** 병목이 rps가 아니라 요청 무게였다 — 최상위 단 days 240(0807 #6) |
> | #14·#15 F19-P·F19-S | AND 강등으로 수리 | **후속 결함.** 남은 단일 조건(임계 0.1)이 위 dispatch 노이즈만으로 설 수 있는 거짓 양성이었다(0807 #17) |
> | #19 F25-H | 256→192→128Mi 사다리 | **후속 수리.** 레벨0 256Mi가 실사용의 2배라 무효과 — 192/128/96으로 재구성(0807 #8) |
>
> 여전히 미결: **#6 F09-R**(클린 베이스라인 미확보), **#10 F03-P**(db.workload가
> live_supported:false 스텁이라 인위적 지연 레버 부재), **#24 F02-H**(캐시 무력화
> 재설계 초안만, 실측 전).
>
> 또한 08-07에 **테스트베드 환경 자립성 결함 2건**(commerce 재고 상시 고갈,
> banking transfer flapping)과 **전역 판정 결함 1건**(APM 백분위의 표본 없는 창이
> 0.0으로 들어와 streak을 리셋)이 확정됐다. 이 문서의 여러 "피해 부재"·"신호가
> 요동친다" 판정은 그 아래층 결함의 그림자일 수 있다.
...
| 12 | F15-T1·F15-P·F14-P | **죽은 신호** | 부하를 만드는 프로파일이 없는데 판정이 부하 산출물을 읽음 | **수리** `7427cec` (domain 한정, 목록 비움) |
...
| 16 | F16-H | **여정에 없는 필드를 안전 관측이 읽음** | 부하는 도는데 checkout 필드가 없어 52틱 전부 사용 불가 → 안전 중단 | **수리** `7427cec`+러너 `2b01f43` |
...
| 20 | F15-R | **~~부하 live.json 조기 소멸~~ → 오진. 관측면이 판정 창보다 60초 먼저 죽는 타이밍** | flap 워커 조기 종료(1200s) + min_hold(1260s)가 fault 창 밖 | **수리** `64390fc` |
...
| 29 | F09-H | **~~레버가 지표를 반대로 움직인다~~ → 오진. 질의가 롤링 중 파드 교차 나눗셈** | 실제 heap-160 파드는 0.5085(평시보다 높음) — 0.324는 합성값 | **수리** 러너 `d4d1586` + `c953018` |
...
> **집행 기록 4 (#29 정정)**: F09-H의 "레버가 지표를 반대로 움직인다"는
> **오진이었다.** 파드별 실측 결과 heap-160 파드의 진짜 비율은 0.5085로 평시
> (0.4769)보다 높았고, 러너 tick의 0.324는 질의가 롤링 중 기준 파드 분자를
> 주입 파드 분모로 나눈 합성값이다(VM 5분 staleness가 죽은 파드를 min_hold
> 4분보다 오래 살려 둔다). 질의를 파드별 나눗셈+last_over_time[90s]로 수리
> (러너 `d4d1586`), 성공 0.8→0.70(0.8은 OOM 절벽 위)·p95 동반 조건 제거·
> 비율 바닥 감별자를 Tenured limit 계단 감별자로 교체(`c953018`). 사다리는
> 원래 5단(208/160/128/112/96)이었고 질의만 고치면 끝까지 오를 수 있다.
> 같은 "분자·분모 독립 집계" 패턴의 다른 템플릿은 없음을 전수 확인했다.
> #29 본문은 사료로 남긴다 — 오진의 경위 자체가 교훈이다(ticks의 값 고정은
> 질의 오염 신호다).
>
> **집행 기록 3 (같은 날 심야)**: 조사 에이전트 실측으로 세 갈래를 닫았다.
> ① F15-R(#20)의 "live.json 조기 소멸"은 **오진** — 파일 삭제는 중단 후
> cleanup의 정상 동작이었고, 진짜 원인은 flap 워커가 판정 창보다 60초 먼저
> 죽는 타이밍이다. 워커 수명 연장 + min_hold 18m으로 수리(`64390fc`).
> 08-03의 F05-H·F08-P "조기 소멸"도 같은 오진이었다.
> ② F16-H(#16)는 결함이 둘 — 주입이 자기 관측의 선행 조건(로그인)을 파괴해
> entry_status·write_401_rate 모두 죽는다. 러너 http.entry_health에 domain
> 한정을 추가하고(`2b01f43`) 관측을 상주 baseline 문서로 옮겼다(`7427cec`).
> ③ #12의 미수리 3종(F14-P·F15-P·F15-T1)도 같은 domain 한정으로 해소 —
> 허용 목록은 빈 집합으로 고정했고, companion 없는 컨트롤러의 부하 관측에
> domain을 강제하는 가드를 추가했다. 전부 미배포.
>
