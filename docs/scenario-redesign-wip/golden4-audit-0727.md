...
> **구조·강제는 완료, 서술은 미완 (2026-07-28).**
>
> 완료: 보류 아닌 34종 전건에 **`injected_fault`**를 기록했다. 컨트롤러의 승인 프로필·레벨 파라미터에서 기계 도출하며, 다음 3건을 CI가 강제한다 — ①전건이 주입을 진술하는가 ②그 진술이 라이브 컨트롤러와 일치하는가(정답지가 실제로 주입된 적 없는 결함을 기술하면 데이터셋이 허구를 채점한다) ③두 시나리오가 같은 주입을 주장하지 않는가. ③이 바로 F21-P/F09-R 결함을 작성 시점에 잡아냈을 검사다(현재 충돌 0 — F21-P 보류로 해소). `code_anchor`의 `파일:라인` 실재 검증도 함께 넣었다.
>
> **미완**: `root_cause{target_kind,target_id,mechanism,code_anchor}` · `must_support` · `must_rule_out` · `distinguishing_evidence`의 **시나리오별 서술이 33건 남았다**(완비는 F24-Q 1건). 이는 시나리오마다 주입면·앱 코드 경로·형제 시나리오와의 감별축을 읽어야 하는 개별 조사 작업이며, 급히 채우면 **틀린 정답지**가 되어 데이터셋에 가장 해롭다. 템플릿 정본과 기계 검증은 준비됐으므로 남은 것은 조사·집필뿐이다.
