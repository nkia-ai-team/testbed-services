...
| B4 | **프로브 질의 ↔ 대상 표의 인덱스** | **없음** | ⚠ EXPLAIN 검사 가능 | **프로브도 워크로드다.** F23-R 프로브가 인덱스를 못 타 15초마다 161MB를 읽어 shared_buffers(128MB)를 밀어냈다(§20). banking 프로브는 같은 자릿수 표인데 인덱스가 있어 cost 6이다 — 규모가 아니라 설계 주의의 차이다. 새 프로브는 **실행 계획과 버퍼 영향을 보고** 붙일 것 |
...
1-b. **B4 프로브 질의 실행 계획** — 신설·기존 프로브가 Seq Scan 을 타는지 한 번 훑기. F23-R 이 이미 물렸고 다른 프로브는 아직 아무도 안 봤다.
