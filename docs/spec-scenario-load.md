...
- **R4-1. 강도는 실측 포화점에서 계산한다 (2026-07-15).** 임계형 부하
  시나리오의 강도는 배수 추측이 아니라 **용량 프로브로 실측한 포화점(절대
  RPS)** 에서 출발한다. 최종 양성 강도는 미실측 배수로 외삽하지 않고,
  **실제로 R0 사용자 영향을 반복 재현하면서 전면 붕괴 전인 가장 낮은 명확한
  양성 구간**을 쓴다. 최초 무릎은 경계·약한 양성 또는 calibration 기준으로
  별도 보존하고, golden 양성은 노이즈와 구분되는 재현 가능한 증상 강도를
  사용한다. 포화점×1.5~2는 다음 탐색 대역을 정하는 초기값일 뿐 golden 강도가
  아니다. 흡수형 음성은 무릎의 ×0.5~0.6을 시작점으로 실측한다.
  배수(intensity_multiplier)는
  운영자 서사용 표기로 유지하되, `injection.parameters`에 목표 절대
  RPS와 실행 시 achieved RPS를 함께 기록한다(§8 실측·§9.3 검토 반영).
  포화점은 경로·여정 믹스별 숫자이며 코드 재배포·데이터 변화 시 낡는다 —
  테스트베드 대변경 후 재측정한다. 방법과 실측값은 §8.
- **R6. 시나리오 간 간격.** 시나리오 사이에 **최소 1시간의 정상 구간**을
  ...
  `multiple_concurrent_anomalies`)과 근접 시간 이종 장애는 한 시나리오 안에서
  명시적으로만 설계한다.
- **R6-1. 동시·근접 장애는 하나의 합성 시나리오로 실행한다.** 두 독립 시나리오를
  ...
  `[t1-2h,t2+45m]`를 따른다. 한쪽 cleanup 실패 시 전체 시나리오를 실패로 처리하고
  다음 시나리오를 시작하지 않는다.
...
brownout이 반복 관측된 s80을 명확한 양성 강도로 사용한다. 이는 R4-1의
“가장 낮은 명확한 양성” 규칙에 따른 선택이며 s70을 건강 구간으로 재분류한다는
뜻은 아니다.
