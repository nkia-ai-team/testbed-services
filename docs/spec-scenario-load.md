...
- **R9. 이상감지 기대값 기입 + 사후 검증.** golden 레코드에 RCA 원인 외에
  **어느 대상(target)의 어느 메트릭/로그가 주입 후 몇 분 내에 이상으로
  ...
  확인하는 **검증 패스**(§5)를 승격 조건으로 둔다. 검증 실패 시 golden을
...
injection:
  ...
  parameters:
    load:
      ...
      intensity_multiplier: 5            # baseline 대비 배수 (기본 표현)
      # absolute_rps: 50                 # 임계 도달형 시나리오만 오버라이드 (§6 미결)
...
- surge 강도의 기본 표현: baseline 대비 배수를 기본, 절대 RPS를 임계
  도달형(풀 고갈 등) 전용 오버라이드로 두는 안 — 확정 전.
- 음성(negative) 부하 시나리오 패턴: "부하가 늘었지만 장애가 아닌" 경우
  (diurnal 피크 수준의 완만한 증가, surge를 시스템이 잘 버틴 경우 등).
  이상감지 false positive와 RCA 과확신("트래픽 증가가 원인") 채점용 —
  [시나리오 설계 §5](spec-scenario-design.md) 음성 규율의 부하 버전으로 별도
  설계 필요.
