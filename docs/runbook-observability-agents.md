...
tags:
  ...
  - observability
  - lucida-next
summary: 109 테스트베드에 lucida-next(AP 119) 관측 4계층을 붙이는 실측 절차. 계층별 등록 방식, 실값, 함정, 검증 방법 포함.
---
...
| 항목 | 값 |
|---|---|
| AP 서버 (lucida-next) | `192.168.230.119` — 웹 UI `:13000`, API `:18080`, OTLP gRPC `:14317`, KCM gRPC `:7575`, VictoriaMetrics `:18428` |
...
| 메트릭 저장소 | VictoriaMetrics 단일(vmsingle). 검증은 `119:18428/api/v1/query` 직접 질의가 가장 확실 |
...
- **핵심 사실**: 이 lucida 빌드에는 Polestar Java 전용 agent jar가 없다
  ...
  **OTel javaagent가 `service.name`을 달아 AP `119:14317`로 직접 전송**하면
...
- **manifest hook** (각 앱 Deployment): apm-agent hostPath 볼륨(`/opt/polestar10/apm`)
  ...
  `OTEL_SERVICE_NAME`, `OTEL_EXPORTER_OTLP_ENDPOINT=http://192.168.230.119:14317`).
...
1. **비대칭 라우팅 해결 (필수)** — AP(230.x — 현 119)의 SYN은 VM 브리지 NIC(enp6s0)로
...
toleration Exists). master가 AP `119:7575`(gRPC)로 push.
...
helm install kcm-agent ~/kcm/kcm-agent -n kcm-monitoring --create-namespace \
  ...
  --set kcm.addr=192.168.230.119:7575 \
...
1. **토큰은 필수이며 target 삭제 시 즉시 수집이 끊긴다.**
   ...
   허용했으나, 07-11 커밋으로 **폐지**(fail-closed)됐고 서버는 07-13 저녁
...
AP는 수시로 업데이트·이관되는 개발 스택이라 관측이 조용히 끊길 수 있다
(2026-07-14 KCM 사례: 서버측 인증 정책 변경으로 하룻밤 새 중단).
알람 튜닝·시나리오 실행 등 관측에 의존하는 작업 전에 4계층을 훑는다.

```bash
BASE=http://192.168.230.119:18080/api/v1
VM=http://192.168.230.119:18428
