...
- **CH ✅**: `col + INTERVAL N DAY`가 DateTime64(3/6/9, UTC)에서 동작(scratch
  ...
  서브초 정밀도 보존.
- **미검증 복잡성 — 머티리얼라이즈드 뷰**: `agg_service_golden_signals`·
  `.inner_id.*`·`dpm_ch_query_event_summary_1m`가 base 테이블 insert로 트리거되는
  MV다. 개별 `INSERT SELECT` 재적재는 MV 재집계/중복 위험. **쟁점 ①=전용 스택 +
  볼륨 복원**을 택하면 볼륨(clickhouse-data)이 base+MV 상태를 통째로 가져오므로
  per-table shift 대신 **벌크 shift**가 되어 이 문제를 우회한다 — **쟁점 ①과 ②는
  연동**된다(볼륨 복원 후 대량 shift 방법은 별도 PoC 필요).

**남은 미결**: 볼륨 복원본에 대한 대량 shift(수백만 행)의 방법·비용, PG TTL shift.
...
- [ ] golden 스냅샷 캡처 스크립트(PG 상태 덤프 + 볼륨 4종 + offset), 1회 실행.
- [ ] 초기화(복원) 스크립트: 격리 스택 재생성 + golden 적재 + time-shift + trainer 동결.
- [ ] 전용 격리 스택 정의(compose)와 에이전트 재배선(쟁점 ① 확정 후).
- [ ] time-shift PoC(VM export/import + CH INSERT SELECT) 비용·정확도 검증(쟁점 ②).
- [ ] runner 통합: 시나리오 경계에서 초기화 호출 + 완료 게이트 + 실패 fail-closed.
- [ ] preflight에 "golden 복원 정합"(모델 버전 고정·trainer 동결 확인) 편입.

## 10. 미해결 질문

- `log_drain_templates`가 0행인 이유(비활성? 다른 저장?) — 로그 신규성 탐지 경로 확인.
- golden 시점 선정 기준(어느 정상 구간을 "가장 깨끗한 학습 상태"로 볼지).
- trainer 동결 방법(autoloop 정지 API vs 컨테이너 정지 vs 모델 버전 핀).
