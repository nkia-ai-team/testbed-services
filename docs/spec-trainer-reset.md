...
- [ ] **runner 통합**: 시나리오 경계(주입 직전)에서 restore 호출 + 완료 게이트 +
      실패 fail-closed, 평가 후 `--thaw`. R6 30분 clean gap이 재워밍 담당.
- [ ] **preflight 편입**: "golden 복원 정합"(observer health·trainer 동결 확인).
- [ ] golden 세트 캡처 운영(시간대별 N장, §8) + golden 윈도 길이·필터 튜닝(VM 138MB/5m).
- [ ] 복원 스크립트 라이브 검증(안전 창) + 회귀 테스트(test-*.sh 관례).
