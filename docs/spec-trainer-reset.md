...
> **[2026-07-24 개정 — 연속 사이클 v3 정합]** [캡처 계약 §2.2]
> (spec-eval-data-capture.md)의 연속 사이클 큐에서는 시점이 다음처럼 바뀐다:
> **golden 복원은 사이클 시작**(주입 직전 ✗)에 수행하고, 이어지는 **정상 2시간
> 동안 trainer를 가동 상태로 학습**시킨다(정상만 보므로 무오염, 매 사이클
> "golden + 갓 학습 2h"로 동일 재현). **동결은 buffer 시작(t1−10m)**, 해동은
> 캡처 완료 후다. §7.2의 time-shift 조립(assembled/)은 v3에서 실제 연속
> 시간축으로 대체되어 캡처 경로에서는 쓰이지 않는다(골든 복원 자체의
> 시간대 정렬 원리는 §8 그대로 유지).
