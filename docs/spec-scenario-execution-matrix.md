...
| F01-G | 짧은 external delay | K/kubectl → commerce MockServer API | retry가 흡수할 지연 범위 | 같은 MockServer | ready — 1/3/5초 adaptive ladder와 5xx·업무 guardrail 고정 (음성, v3 재캡처 복원 07-24) |
| F02-H | storage IO stress | tb-w1/ssh | tb-w1 `/dev/vda1`(commerce PG PVC 실측 위치); commerce NodePort 30080 | 같은 worker의 fio 종료·생성 파일 삭제 | ready(2026-07-28) — fio 설치·사다리 전환·host.disk_io_utilization 신설로 승격 |
...
| F05-G | invalid image rollout | K/kubectl → `rca-testbed-commerce/testbed-payment` | 새 replica만 실패, 기존 replica 유지 | K에서 snapshot한 원 image 복원 | ready — target/image 정본과 ImagePullBackOff 관측 고정 (음성, v3 재캡처 복원 07-24) |
...
| F10-H | MySQL IO saturation | tb-w3/ssh | tb-w3 `/dev/vda1`(food MySQL PVC 실측 위치) | 같은 worker stress 종료 | ready(2026-07-28) — 좌표는 배치 고정 이후 실측 확정 |
| F10-P | Oracle IO saturation | tb-w2/ssh | tb-w2 `/dev/vda1`(banking Oracle PVC 실측 위치 — 07-28 배치 고정으로 tb-w1에서 이사, PV 재프로비저닝) | 같은 worker stress 종료 | ready(2026-07-28) |
...
| F15-P | node 복합 자원 고갈(CPU+메모리 동시) | tb-w2/ssh | tb-w2의 banking pods와 Oracle | worker stress-ng 종료 | ready(2026-07-28) — 배치 고정으로 '공용 노드' 전제가 소멸해 복합 자원 고갈로 재정의 |
...
| F18-P | outbox relay 정지 → 원장/알림 silent lag | K/kubectl app.control outbox_relay_control.enabled=0 (rca-testbed-banking, 무재기동) | 동기 이체는 정상(2xx 유지), outbox 미발행 적체만 증가 — F04-R(lag 증가)과 감별 | K enabled=1 복원(무재기동) | ready — env 토글은 maxSurge=0 롤아웃이 감별자를 켜서 폐기(0804 #13), DB 플래그로 재설계(08-06) |
...
| F21-Q | food order-service Tomcat 200 스레드풀 포화(restaurant 상류지연, save 前 hop이라 Hikari 미보유) | W/ssh host.stress(worker-w3, cpu) + R/ssh load.north_south companion(order-surge.js, target_rps=70) | order_busy_threads(jvm_daemon_thread_count)↑ + order_hikari_pending==0 + order_p95_latency↑ | W stress 해제 | ready(2026-07-28) — 좌표 w2→w3 정정, busy-thread 신호를 daemon 스레드로 교체 |
| F21-P | core-banking api-service Tomcat 200 스레드풀 포화(transfer 하류지연, DataSource 부재로 풀 자체 없음) | K/kubectl transfer cpu limit 사다리(400m→300m→200m) + R/ssh load.north_south companion(core-banking/loadgen/transfer-heavy-surge.js, target_rps=50) | api_busy_threads(jvm_daemon_thread_count)↑ + api_502_rate<0.05(CB closed, 느린 200) | K 원 limit rollback, R load 종료 | ready(2026-07-31) — 주입을 노드 CPU 압박에서 transfer 전용 CPU limit으로 교체. 노드 압박은 api까지 굶겨 CB를 열었고, 실측 램프(7→97rps)에서 "스레드≥180"과 "오류율<5%"가 겹치는 구간이 없었다 |
