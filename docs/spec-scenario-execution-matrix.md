...
| F21-P | core-banking api-service Tomcat 200 스레드풀 포화(transfer 하류지연, DataSource 부재로 풀 자체 없음) | W/ssh host.stress(worker-w2, cpu) + R/ssh load.north_south companion(core-banking/loadgen/transfer-heavy-surge.js, target_rps=90) | api_busy_threads(jvm_daemon_thread_count)↑ + api_502_rate<0.05(CB closed, 느린 200) | W stress 해제 | ready(2026-07-28) — 좌표 w3→w2 정정, busy-thread 신호를 daemon 스레드로 교체 |
