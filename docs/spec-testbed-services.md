...
| 비동기 | **Redis Streams** (`StringRedisTemplate.opsForStream`, consumer group). 기존엔 plopvape에만 존재 → 신설계는 전 도메인에 도입 |
...
- `spring.application.name`은 순수 모듈명(`order-service`)으로 두고, OTel용
  ...
  재정의한다(기존 실측: `food-delivery-order`, `plopvape-order`).
...
- 기존 실측값: `lucida.organizationId=${POLESTAR_ORG_ID},lucida.groupId=<domain>`
  (food는 `${POLESTAR_ORG_ID}` placeholder + `build-and-deploy.sh` envsubst; plopvape는
...
### 4.1 commerce (PostgreSQL) — plopvape-shop 기반
...
- 개조: plopvape-shop을 commerce로 리네이밍(또는 신규 폴더) — §10 결정.
...
- 개조: (a) PG→MySQL, (b) **Redis 비동기 경로 신규 추가**(기존 food엔 없음 — plopvape
...
| 기존 자산 | 신설계에서 |
| --- | --- |
| plopvape-shop (PG + Redis Streams, 5 svc + nginx) | **commerce** 베이스(거의 그대로, 리네이밍) |
...
- **commerce = plopvape-shop을 commerce로 리네이밍**(PG + Redis, 베이스 그대로).
